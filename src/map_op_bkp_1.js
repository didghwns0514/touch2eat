...
let app = {
  ...
  mapScriptReady: function () {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(app.gotPosition, app.failPosition, options);
    } else {
      //not supported
  ...
  runSearchRequest: function () {
    ...
    let ev = {
      latLng: {
        lat: function () {
          return app.currMapCenter.latitude;
        },
        lng: function () {
          return app.currMapCenter.longitude;
        },
      },
    };
    app.addMarker(ev);
  ...
  updateTable: function () {
    ...
    let compareFunction = (a, b) => {
      let tmpA =
        (a.rating * Math.sqrt(a.user_ratings_total)) /
        Math.pow(Math.hypot(a.geometry.location.lat() - setPoint.lat, a.geometry.location.lng() - setPoint.lng), 1); // (a.price_level + 1);
      let tmpB =
        (b.rating * Math.sqrt(b.user_ratings_total)) /
        Math.pow(Math.hypot(b.geometry.location.lat() - setPoint.lat, b.geometry.location.lng() - setPoint.lng), 1); // (b.price_level + 1);
  ...
  findRoutetoPlace: function () {
    ...
    if (app.currentMarker === null) {
      ...
      console.log(
        `app.currentMarker.position.lat, app.currentMarker.position.lng : ${app.currentMarker.position.lat()}, ${app.currentMarker.position.lng()}`
      );
      ...
      if (app.currentLocMarker === null) {
        let ev = {
          latLng: {
            lat: function () {
              return app.currLoc.latitude;
            },
            lng: function () {
              return app.currLoc.longitude;
            },
          },
        };
        app.addMarker(ev);
    ...
    if (app.currentMarker !== null) {
      ...
      if (queryResult === null) {
        ...
        dispText += `<h1>${queryResult.name}</h1>
                            ...
                            <p>more info : <a href=https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(
                              queryResult.name
                            )}&query_place_id=${queryResult.place_id}>Link</a></p>

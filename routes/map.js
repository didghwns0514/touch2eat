...
dotenv.config({
  path: path.resolve(process.cwd(), process.env.HEROKU === "true" ? ".env" : ".env.dev"),
});
...
router.route("/{1}").get((req, res, next) => {
...
router.route("/form").post((req, res, next) => {
...
router.route("/real").get((req, res, next) => {
